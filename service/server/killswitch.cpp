...
bool KillSwitch::enablePeerTraffic(const QJsonObject &configStr) {
    ...
    config.m_primaryDnsServer = configStr.value(mugen::configKey::dns1).toString();

    // We don't use secondary DNS if primary DNS is MugenDNS
    if (!config.m_primaryDnsServer.contains(mugen::protocols::dns::mugenDnsIp)) {
        config.m_secondaryDnsServer = configStr.value(mugen::configKey::dns2).toString();
    }
    ...
    for (const QJsonValue &i : configStr.value(mugen::configKey::splitTunnelApps).toArray()) {
        if (!i.isString()) {
            break;
        }
    ...
    for (auto dns : configStr.value(mugen::configKey::allowedDnsServers).toArray()) {
        if (!dns.isString()) {
            break;
        }
    ...
    // killSwitch toggle
    if (QVariant(configStr.value(mugen::configKey::killSwitchOption).toString()).toBool()) {
...
bool KillSwitch::enableKillSwitch(const QJsonObject &configStr, int vpnAdapterIndex) {
    ...
    dnsServers.append(configStr.value(mugen::configKey::dns1).toString());

    // We don't use secondary DNS if primary DNS is MugenDNS
    if (!configStr.value(mugen::configKey::dns1).toString().contains(mugen::protocols::dns::mugenDnsIp)) {
        dnsServers.append(configStr.value(mugen::configKey::dns2).toString());
    }
