...
void CoreController::initAppleController()
{
    ...
    QTimer::singleShot(0, this, [this]() { MugenVPN::toggleScreenshots(m_appSettingsRepository->isScreenshotsEnabled()); });
#endif
}
...
void CoreController::updateTranslator(const QLocale &locale)
{
    ...
    QDirIterator it(":/translations", QStringList("mugenvpn_*.qm"), QDir::Files);
    while (it.hasNext()) {
    ...
    const QString translationFilePrefix = QString(":/translations/mugenvpn_") + lang;
    QString strFileName = QString(":/translations/mugenvpn_%1.qm").arg(locale.name());
    ...
    if (m_translator->load(strFileName)) {
        ...
        if (m_translator->load(QString(":/translations/mugenvpn_en.qm"))) {
            QCoreApplication::installTranslator(m_translator);
        }
    }
