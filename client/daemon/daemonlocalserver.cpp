...
constexpr const char* TMP_PATH = "/tmp/mugenvpn.socket";
constexpr const char* VAR_PATH = "/var/run/mugenvpn/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("mugenvpn")) {
    logger.debug() << "/var/run/mugenvpn seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("mugenvpn")) {
    logger.warning() << "Failed to create /var/run/mugenvpn";
    return TMP_PATH;
  }

  if (chmod("/var/run/mugenvpn", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/mugenvpn";
    return TMP_PATH;
  }
