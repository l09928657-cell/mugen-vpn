...
XrayConfigSnapshot XrayConfigSnapshot::fromJson(const QJsonObject &json)
{
    ...
    s.serverConfig = mugen::XrayServerConfig::fromJson(json.value("serverConfig").toObject());
    return s;
}
...
void XrayConfigSnapshotsModel::createFromCurrent(const mugen::XrayServerConfig &serverConfig)
{
...
mugen::XrayServerConfig XrayConfigSnapshotsModel::applyConfig(int index) const
{
    if (index < 0 || index >= m_configs.size()) {
        return mugen::XrayServerConfig {};
    }
...
QString XrayConfigSnapshotsModel::buildDisplayName(const mugen::XrayServerConfig &cfg)
{
...
void XrayConfigSnapshotsModel::applyConfigToCurrentModel(int index)
{
    ...
    mugen::XrayServerConfig cfg = applyConfig(index);
    if (cfg.port.isEmpty()) {
