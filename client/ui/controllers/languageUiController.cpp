...
QString LanguageUiController::getCurrentSiteUrl(const QString &path) const
{
    ...
    if (locale.language() == QLocale::Russian) {
        return "https://storage.googleapis.com/mugen/mugen.org" + (path.isEmpty() ? "" : (QString("?m-path=/%1").arg(path)));
    }
    return QString("https://mugen.org") + (path.isEmpty() ? "" : (QString("/%1").arg(path)));
}
...
QString LanguageUiController::getCurrentDocsUrl(const QString &path) const
{
    auto locale = m_settingsController->getAppLanguage();
    if (locale.language() == QLocale::Russian) {
        return "https://storage.googleapis.com/mugen/docs" + (path.isEmpty() ? "" : (QString("?m-path=/%1").arg(path)));
    }
    return QString("https://docs.mugen.org") + (path.isEmpty() ? "" : (QString("/%1").arg(path)));
}
